...
//BBCBlockScanner ontology的区块链扫描器
type BBCBlockScanner struct {
...
//NewBBCBlockScanner 创建区块链扫描器
func NewBBCBlockScanner(wm *WalletManager) *BBCBlockScanner {
	bs := BBCBlockScanner{
...
func (bs *BBCBlockScanner) SetRescanBlockHeight(height uint64) error {
	height = height - 1
	if height < 0 {
...
//ScanBlockTask 扫描任务
func (bs *BBCBlockScanner) ScanBlockTask() {
...
//ScanBlock 扫描指定高度区块
func (bs *BBCBlockScanner) ScanBlock(height uint64) error {
...
func (bs *BBCBlockScanner) scanBlock(height uint64) (*Block, error) {
...
func (bs *BBCBlockScanner) ScanTxMemPool() {
...
func (bs *BBCBlockScanner) RescanFailedRecord() {
...
func (bs *BBCBlockScanner) newBlockNotify(block *Block, isFork bool) {
...
func (bs *BBCBlockScanner) BatchExtractTransaction(blockHeight uint64, blockHash string, txs []string, memPool bool) error {
...
func (bs *BBCBlockScanner) extractRuntime(producer chan ExtractResult, worker chan ExtractResult, quit chan struct{}) {
...
func (bs *BBCBlockScanner) ExtractTransaction(blockHeight uint64, blockHash string, txid string, scanAddressFunc openwallet.BlockScanAddressFunc, memPool bool) ExtractResult {
...
func (bs *BBCBlockScanner) extractTransaction(trx *Transaction, result *ExtractResult, scanAddressFunc openwallet.BlockScanAddressFunc) {
	var (
		success = true
	)
	createAt := time.Now().Unix()
	anchor, err := bs.wm.Client.getAnchor()
	if err != nil {
		success = false
		return
	}
	if trx == nil || trx.Anchor != anchor {
		//记录哪个区块哪个交易单没有完成扫描
		success = true
	} else {
...
func (bs *BBCBlockScanner) newExtractDataNotify(height uint64, extractData map[string]*openwallet.TxExtractData) error {
...
func (bs *BBCBlockScanner) DeleteUnscanRecordNotFindTX() error {
...
//func (bs *BBCBlockScanner) SaveRechargeToWalletDB(height uint64, list []*openwallet.Recharge) error {
//
//	for _, r := range list {
//
...
func (bs *BBCBlockScanner) GetCurrentBlockHeader() (*openwallet.BlockHeader, error) {
	var (
...
func (bs *BBCBlockScanner) GetScannedBlockHeader() (*openwallet.BlockHeader, error) {
	...
	if blockHeight == 0 {
		blockHeight, err = bs.wm.GetBlockHeight()
		if err != nil {
			bs.wm.Log.Errorf("BBC GetBlockHeight failed,err = %v", err)
			return nil, err
		}
...
func (bs *BBCBlockScanner) GetScannedBlockHeight() uint64 {
	localHeight, _, _ := bs.wm.Blockscanner.GetLocalNewBlock()
	return localHeight
}

func (bs *BBCBlockScanner) ExtractTransactionData(txid string, scanTargetFunc openwallet.BlockScanTargetFunc) (map[string][]*openwallet.TxExtractData, error) {
...
func (bs *BBCBlockScanner) GetSourceKeyByAddress(address string) (string, bool) {
	bs.Mu.RLock()
	defer bs.Mu.RUnlock()
...
func (bs *BBCBlockScanner) GetLocalNewBlock() (uint64, string, error) {
...
func (bs *BBCBlockScanner) SaveLocalNewBlock(blockHeight uint64, blockHash string) error {
...
func (wm *WalletManager) GetBlock(hash string) (*Block, error) {
	return nil, openwallet.Errorf(openwallet.ErrUnknownException, "Get block by block hash is not supported!")
}
...
func (bs *BBCBlockScanner) GetBalanceByAddress(address ...string) ([]*openwallet.Balance, error) {
	...
	for _, addr := range address {
		balance, err := bs.wm.Client.getBalance(addr, anchor)
		if err != nil {
			return nil, err
		}
...
func (bs *BBCBlockScanner) GetTransactionsByAddress(offset, limit int, coin openwallet.Coin, address ...string) ([]*openwallet.TxExtractData, error) {
...
//Run 运行
func (bs *BBCBlockScanner) Run() error {
...
////Stop 停止扫描
func (bs *BBCBlockScanner) Stop() error {
...
//Pause 暂停扫描
func (bs *BBCBlockScanner) Pause() error {
...
//Restart 继续扫描
func (bs *BBCBlockScanner) Restart() error {
...
func (bs *BBCBlockScanner) setupSocketIO() error {
